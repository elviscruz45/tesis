...
import { ChatGPTDisenoInvestigacion } from "./ChatGPTDisenoInvestigacion";
...
export default async function RetriveDisenoGeneralInvestigacion({
  open,
  setOpen,
  contentData,
  sectionName,
  ...
  return (
    <>
      ...
      <br />
      <ChatGPTDisenoInvestigacion
        titulo={titulo?.title}
        poblacion={poblacion?.title}
        userId={userId}
        seccion={"Diseno General de la Investigación"}
      />
    </>
  );
}
